...
def test_generate_task_id_valid_scan():
    ...
    # Check format: YYYYMMDDTHHMMSS.ffffff-scan (ISO8601 basic, no hyphens/colons)
...
def test_generate_task_id_valid_rip():
    ...
    # Check format: YYYYMMDDTHHMMSS.ffffff-rip (ISO8601 basic, no hyphens/colons)
...
def test_generate_task_id_timestamp_format():
    """Test that the timestamp in task ID is a valid ISO8601 basic format."""
    task_id = generate_task_id("scan")

    # Extract timestamp part (everything before the last dash)
    timestamp_str = task_id.rsplit("-", 1)[0]

    # Should be parseable as ISO8601 basic format (YYYYMMDDTHHMMSS.ffffff)
    # Parse manually since strptime needs explicit format for basic ISO8601
    assert len(timestamp_str) == 22  # 8 + 1 (T) + 6 + 1 (.) + 6
